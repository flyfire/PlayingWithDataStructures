 ...
 * Desc:
 */
public class RadixSort extends SortingAlgorithms {
    @Override
    ...
    // arr 待排数组
    // bits 每次处理的二进制位数 可选值：1,2,4,8,16
    // mask 每次移动bits个二进制位后，使用mask取出最低的bits位
    // Time: O(32/b * n), Space: O(n + 2^b)
    private void radixSort(int[] arr, int bits, int mask) {
